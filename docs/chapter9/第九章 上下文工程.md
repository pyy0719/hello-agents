...
1. 本章介绍了上下文工程与提示工程的区别。请分析：
   ...
   **参考答案：**
   ...
   （1）上下文腐蚀指的是：随着上下文窗口中信息越来越多，模型可见的信息虽然增加了，但有效信息密度下降，模型更容易受到无关内容、过期内容、重复内容或冲突内容的干扰，最终导致回答质量下降。它不是简单的"上下文不够长"问题，而是"上下文里有太多低价值或互相干扰的信息"问题。

   即使模型支持100K或200K上下文，也仍然需要管理上下文，原因包括：

   - 注意力资源并不是无限的，长上下文中关键信息可能被噪声淹没。
   - 长上下文会增加调用成本和延迟，不适合所有生产场景。
   - 过期信息可能与当前任务状态冲突，导致模型做出错误判断。
   - 重复信息会放大某些结论的权重，造成模型偏向错误方向。
   - 大窗口只能缓解容量问题，不能自动解决相关性、结构化和优先级问题。

   因此，上下文工程的目标不是"尽可能多塞内容"，而是"在有限上下文中放入当前任务最有用、最新、可信、结构清晰的信息"。

   （2）代码审查助手的两种策略对比如下：

   | 策略 | 优点 | 缺点 | 适用场景 |
   | --- | --- | --- | --- |
   | 一次性加载所有文件 | 实现简单；模型能看到全局信息；适合发现跨文件关系 | 成本高；容易上下文腐蚀；大量无关文件干扰判断；文件较多时可能超出窗口 | 小型项目、文件数量少、需要全局快速概览的场景 |
   | JIT按需检索文件 | 上下文更聚焦；成本和延迟更可控；适合大型代码库；能根据问题逐步探索 | 需要工具调用和检索策略；如果检索不准可能漏掉关键文件；实现复杂度更高 | 中大型项目、长程代码审查、需要多轮深入分析的场景 |

   对50个文件的代码库，更推荐JIT上下文策略。比较合理的做法是：先读取目录结构、依赖文件、入口文件和变更文件，建立项目地图；再根据审查目标按需读取相关模块；最后用ContextBuilder选择最相关的代码片段、历史笔记和用户要求，构造本轮审查上下文。
   ...
   ```text
   你必须按照以下12个步骤审查每一份代码，不管用户问题是什么，都必须检查数据库、缓存、安全、性能、测试、日志、部署、文档、异常处理、类型注解、代码风格和依赖版本。
   ```

   问题在于它限制过死，导致模型缺乏任务适应性。即使用户只问一个很小的函数 bug，模型也可能输出冗长、低相关的审查报告。
   ...
   问题在于它没有说明审查标准、输出结构、风险优先级、是否可以调用工具，也没有告诉模型如何处理不确定性。

   更合理的平衡点是：系统提示给出稳定原则和边界，具体上下文由每轮任务动态补充。例如：

   ```text
   你是代码审查助手。优先发现正确性、安全性、性能和可维护性问题。回答时先列高风险问题，再给出修改建议。遇到不确定信息时说明假设，并优先通过工具读取相关文件，不要凭空推断。
   ```

   这样既不会把流程写死，也不会让模型缺少行为约束。
...
2. GSSC（Gather-Select-Structure-Compress）流水线是本章的核心技术。请深入思考：
   ...
   **参考答案：**
   ...
   （1）GSSC四个阶段的职责和失效影响如下：

   | 阶段 | 主要职责 | 失效后的影响 |
   | --- | --- | --- |
   | Gather | 收集候选信息，如系统指令、用户问题、历史对话、笔记、检索结果、工具输出 | 信息源不完整，模型缺少关键背景，容易答非所问或重复询问 |
   | Select | 根据相关性、时效性、优先级和token预算选择信息 | 如果选错，会把无关信息放进上下文，导致上下文腐蚀；如果漏选，会丢失关键证据 |
   | Structure | 将信息组织成清晰区域，如任务、约束、证据、历史、输出要求 | 模型难以区分指令、事实和参考材料，可能混淆优先级 |
   | Compress | 在超预算时压缩低优先级信息，保留核心语义 | 过度压缩会丢失细节；摘要不准确会引入幻觉；压缩不足会超出预算或保留噪声 |

   举例来说，如果Select阶段选择了大量无关旧笔记，模型可能会围绕过期问题展开分析；如果Compress阶段把错误日志压缩成"系统出现异常"，就会丢掉异常类型、文件路径和堆栈位置，后续调试会变得困难。

   （2）可以为ContextBuilder增加上下文质量评估功能。核心思想是：构建上下文之后，不只返回文本，还返回质量指标和优化建议。

   示例设计：

   ```python
   class ContextQualityEvaluator:
       def evaluate(self, packets, final_context, query):
           total_tokens = sum(p.token_count for p in packets)
           useful_tokens = sum(
               p.token_count for p in packets
               if p.relevance_score >= 0.6
           )
           information_density = useful_tokens / max(total_tokens, 1)
           avg_relevance = sum(p.relevance_score for p in packets) / max(len(packets), 1)

           required_sections = ["[Role & Policies]", "[Task]", "[Evidence]", "[Output]"]
           completeness = sum(
               1 for section in required_sections
               if section in final_context
           ) / len(required_sections)

           suggestions = []
           if information_density < 0.6:
               suggestions.append("上下文信息密度偏低，建议提高min_relevance或减少历史消息数量。")
           if avg_relevance < 0.7:
               suggestions.append("候选信息整体相关性不足，建议优化检索查询或增加任务关键词匹配。")
           if completeness < 1.0:
               suggestions.append("上下文结构不完整，建议补充任务、证据或输出要求。")
   ...
   在ContextBuilder中可以这样集成：

   ```python
   context = builder.build(query)
   quality_report = evaluator.evaluate(selected_packets, context, query)
   return context, quality_report
   ```

   这个评估功能的价值是让上下文构建过程可观测、可调试。生产系统中不能只看最终回答，还需要知道"模型到底看到了什么上下文，以及这些上下文质量如何"。

   （3）LLM摘要并不总是最优方案。以下情况中，简单截断或滑动窗口可能更合适：

   - 日志、堆栈、代码片段等内容要求精确保留，LLM摘要可能丢失关键字符或行号。
   - 实时系统对延迟敏感，不适合再调用一次LLM做摘要。
   - 内容已经高度结构化，例如JSON、表格、配置文件，规则压缩更稳定。
   - 只关心最近若干轮对话时，滑动窗口足够有效。
   - 成本敏感场景中，LLM摘要会增加额外费用。

   一个混合压缩策略可以这样设计：

   | 信息类型 | 压缩策略 |
   | --- | --- |
   | 系统指令、用户当前任务 | 不压缩，必须完整保留 |
   | 代码片段、错误堆栈、配置文件 | 优先保留原文；必要时按行号截断 |
   | 最近对话 | 使用滑动窗口，保留最近N轮 |
   | 长期历史笔记 | 使用LLM摘要，提取结论、决策、阻塞点 |
   | 搜索结果或文档 | 先规则提取标题、摘要、链接，再必要时LLM压缩 |

   伪代码如下：

   ```python
   def compress_packet(packet):
       if packet.type in ["system", "task"]:
           return packet.content
       if packet.type in ["code", "error_log"]:
           return preserve_head_tail_with_line_numbers(packet.content)
       if packet.type == "conversation":
           return sliding_window(packet.content, last_n=6)
       if packet.type in ["note", "document"]:
           return llm_summarize(packet.content)
       return truncate(packet.content, max_tokens=500)
   ```

   这个策略的核心原则是：事实敏感内容少摘要，历史性内容可摘要，当前任务和硬约束不压缩。
...
3. NoteTool和TerminalTool是支持长时程任务的关键工具。基于9.4节和9.5节的内容，请完成以下扩展实践：
   ...
   **参考答案：**
   ...
   （1）笔记自动整理机制可以采用"触发条件 + 分类评估 + 提升合并 + 清理归档"四步。

   触发条件：

   - 临时笔记数量超过阈值，例如20条。
   - 临时笔记累计token超过阈值，例如8000 tokens。
   - 某个任务阶段结束，例如完成一次代码审查或重构。
   - 用户手动触发整理，例如输入"整理最近笔记"。

   分类评估：

   - 按语义聚类，把相似笔记归为同一主题。
   - 判断每条笔记的重要性，例如是否包含结论、决策、阻塞、待办、风险。
   - 判断时效性，过期信息可以降权或归档。
   - 判断是否与项目长期目标有关。
   ...
   | 临时笔记内容 | 处理方式 |
   | --- | --- |
   | 一次性观察、短期调试输出 | 保留为临时笔记或归档 |
   | 明确待办事项 | 提升为任务笔记 |
   | 项目架构、关键约束、长期决策 | 提升为项目笔记 |
   | 多条重复结论 | 合并成一条结构化笔记 |
   | 已解决问题 | 标记为resolved并保留摘要 |
   ...
   临时笔记达到20条
        ↓
   按主题聚类：数据库、测试、重构、依赖问题
        ↓
   提取高价值信息：结论、TODO、风险、决策
        ↓
   生成任务笔记或项目笔记
        ↓
   原临时笔记标记为archived
   ...
   这个机制要注意保留可追溯性。被整理后的新笔记应该记录来源笔记ID，避免整理后无法知道结论来自哪里。

   （2）TerminalTool的路径验证、命令白名单、权限检查是必要基础，但通常不够。原因是：

   - 白名单命令也可能被滥用，例如 `cat .env` 会泄露密钥。
   - 路径验证只能限制访问范围，不能判断文件内容是否敏感。
   - 有些命令本身安全，但参数组合可能危险。
   - 智能体可能误判用户意图，执行破坏性操作。

   因此需要人机协作审批流程。可以把操作分级：

   | 风险级别 | 操作示例 | 处理方式 |
   | --- | --- | --- |
   | 低风险 | `ls`、`pwd`、读取普通源码 | 自动执行 |
   | 中风险 | 读取配置文件、运行测试、安装依赖 | 执行前展示计划，可配置是否自动通过 |
   | 高风险 | 删除文件、修改数据库、读取`.env`、提交代码、推送远程 | 必须人工审批 |
   | 禁止操作 | 泄露密钥、上传私有代码、执行不可逆系统命令 | 直接拒绝 |
   ...
   Agent准备执行命令
        ↓
   风险评估：命令类型、路径、参数、影响范围
        ↓
   生成执行计划：为什么执行、会影响哪些文件、如何回滚
        ↓
   用户确认
        ↓
   执行命令并记录日志
   ...
   对危险操作，审批信息应该具体。例如："将删除 `tmp/cache` 目录下15个文件，不影响源码；可通过git恢复的文件有0个"。不能只问"是否允许执行危险命令"。

   （3）智能代码重构助手可以设计为以下工作流：
   ...
   TerminalTool扫描代码库结构
          ↓
   ContextBuilder构建项目上下文
          ↓
   LLM分析模块边界、重复代码、复杂函数、测试情况
          ↓
   NoteTool记录发现的问题
          ↓
   生成重构计划
   ...
   [输入重构目标]
      |
      v
   [TerminalTool: 探索目录/读取关键文件]
      |
      v
   [NoteTool: 检索历史任务和阻塞点]
      |
      v
   [ContextBuilder: 组装当前任务上下文]
      |
      v
   [LLM: 生成重构计划]
      |
      v
   [NoteTool: 保存 task_state/action/blocker]
      |
      v
   [人工审批计划]
      |
   ...
   [逐项执行重构]
      |
      v
   [TerminalTool: 运行测试/检查结果]
      |
      +---- 测试通过 ----> [NoteTool: 标记完成] ----+
      |                                            |
      +---- 测试失败 ----> [NoteTool: 记录blocker] -+
                                                   |
                                                   v
                                           [生成重构报告]
                                                   |
   ...
   这个助手的关键不是一次性完成所有修改，而是把"分析、计划、执行、验证、记录"做成可追踪闭环。
...
4. 在9.6节的"长时程任务管理"案例中，我们看到了上下文工程在实际应用中的价值。请深入分析：
   ...
   **参考答案：**
   ...
   | 层级 | 工具 | 存放信息 | 特点 |
   | --- | --- | --- | --- |
   | 即时访问 | TerminalTool | 当前需要查看的文件、命令输出、测试结果、日志片段 | 实时、按需、原始信息多 |
   | 会话记忆 | MemoryTool | 当前会话中的用户意图、短期对话、刚刚形成的中间结论 | 生命周期较短，适合维持当前对话连贯性 |
   | 持久笔记 | NoteTool | 项目决策、任务状态、阻塞问题、长期计划、重要结论 | 可跨会话保存，适合长期任务 |

   协调原则：

   - TerminalTool负责"查事实"，不负责长期保存所有输出。
   - MemoryTool负责"当前对话连贯性"，不应存大量原始文件内容。
   - NoteTool负责"长期可复用知识"，只保存提炼后的结论、任务和决策。

   避免冗余和不一致的方法：

   - 原始信息只保留引用，例如文件路径、行号、命令和时间戳，不把整段大文件复制进笔记。
   - 重要结论进入NoteTool时记录来源，例如"来自 tests/test_user.py 的测试失败结果"。
   - 笔记设置状态字段，例如 `active`、`resolved`、`archived`，避免过期信息继续影响模型。
   - 每次构建上下文时优先选择最新、相关、未解决的信息。
   - 对冲突笔记做显式标记，例如"新结论覆盖旧结论"。

   简单说：TerminalTool提供事实，MemoryTool维持当前会话，NoteTool沉淀长期状态，ContextBuilder负责在每轮调用时把三者组合成合适上下文。

   （2）断点续传机制的核心是：在每个关键步骤后保存可恢复状态，并在恢复时校验外部环境是否仍然一致。

   笔记中应该记录的信息包括：

   - 当前任务ID和任务名称。
   - 任务状态：pending、in_progress、blocked、done。
   - 已完成步骤和下一步动作。
   - 涉及的文件路径、函数名、命令、测试结果。
   - 最近一次成功检查点的时间。
   - 当前假设和未解决问题。
   - 如果修改了代码，应记录变更摘要、相关文件和验证结果。

   示例笔记结构：

   ```yaml
   type: task_state
   task_id: refactor_order_service
   status: in_progress
   completed_steps:
     - 分析 process_order 嵌套问题
     - 提取重构方案
     - 新增测试用例草稿
   next_step: 修改 order_service.py 并运行订单相关测试
   files:
     - app/services/order_service.py
     - tests/test_order_service.py
   last_command: pytest tests/test_order_service.py
   last_result: 2 failed, 5 passed
   updated_at: 2025-01-19T16:00:00
   ```
   ...
   检查任务状态和下一步动作
       ↓
   用TerminalTool验证文件是否存在、git状态是否变化、测试结果是否可复现
       ↓
   如果状态一致：继续执行下一步
       ↓
   如果状态不一致：生成恢复报告，请用户确认
   ```
   ...
   断点续传不能只相信笔记，因为外部文件系统可能已经变化。正确做法是"笔记恢复 + 环境验证"。

   （3）任务依赖管理系统可以把每个子任务表示成一个结构化任务节点：

   ```yaml
   task_id: add_email_unique_constraint
   title: 为 User.email 添加唯一约束
   status: pending
   priority: high
   depends_on: []
   blocks:
   ...
   - 只有依赖任务全部完成，当前任务才能进入ready状态。
   - 如果某个任务blocked，依赖它的任务也应暂停。
   - 高优先级任务在同等条件下优先执行。
   - 修改同一文件的任务应避免并行执行，防止冲突。
   - 每次任务状态变化后，重新计算可执行任务列表。

   与NoteTool集成方式：

   - 每个任务是一条 `task_state` 笔记。
   - `depends_on` 字段记录依赖任务ID。
   - `status` 字段记录任务状态。
   - `blocker` 类型笔记可以关联到具体任务。
   - `summary` 操作可以生成当前任务看板。

   调度伪代码：

   ```python
   def get_ready_tasks(notes):
       tasks = [note for note in notes if note["type"] == "task_state"]
   ...
   这个系统让长程任务从"聊天记录式推进"变成"结构化项目管理式推进"。
...
5. 本章多次提到"渐进式披露"（progressive disclosure）的概念。请思考：
   ...
   **参考答案：**
   ...
   （1）以"复杂线上问题调试"为例，渐进式披露可以这样发挥作用：

   ```text
   用户：接口响应时间突然变慢，请帮我排查。
        ↓
   第一步：Agent查看最近日志和监控摘要
        ↓
   发现：慢请求集中在 /api/orders
        ↓
   第二步：Agent读取订单接口代码
        ↓
   发现：接口调用了 order_service 和 product_service
        ↓
   第三步：Agent查看最近数据库慢查询
        ↓
   发现：orders 表缺少 user_id + created_at 联合索引
        ↓
   第四步：Agent检查最近提交
        ↓
   发现：新版本增加了按用户查询订单历史的逻辑
        ↓
   第五步：Agent提出修复方案并记录问题笔记
   ```

   这个过程不是一开始把所有日志、代码、数据库结构和提交记录都塞进上下文，而是每一步根据上一步发现决定下一步看什么。这样可以降低噪声，提高排查效率。
   ...
   - 接口代码告诉Agent应该关注哪些服务。
   - 慢查询告诉Agent瓶颈可能在数据库。
   - Git提交告诉Agent问题可能由哪个改动引入。

   这就是渐进式披露的核心：上下文不是一次性给完，而是在任务推进中逐步揭示。

   （2）探索引导机制可以结合启发式规则和元认知检查。

   启发式规则：

   - 优先查看与用户问题直接相关的入口文件。
   - 优先查看最近修改过的文件。
   - 优先查看错误日志中出现的文件、函数、行号。
   - 对高风险模块优先探索，例如认证、支付、数据写入、权限控制。
   - 如果连续两步没有获得新信息，切换探索方向。
   - 如果当前证据不足，不要直接下结论，先列出缺失信息。

   元认知检查可以让Agent在每一步后问自己：
   ...
   | 动作类型 | 目的 |
   | --- | --- |
   | 扫描 | 建立整体结构，如查看目录和入口文件 |
   | 聚焦 | 读取与问题直接相关的文件 |
   | 验证 | 运行测试、复现错误、检查日志 |
   | 对比 | 查看最近提交、版本差异、历史笔记 |
   | 收敛 | 形成结论、提出修复方案 |

   好的探索引导机制要避免两种问题：一是盲目深入细节，二是证据不足就过早收敛。

   （3）渐进式披露与一次性加载所有上下文的对比如下：

   渐进式披露更适合：

   - 大型代码库调试：信息量大，必须按错误线索逐步定位。
   - 长程研究任务：资料来源多，需要边读边调整研究方向。
   - 多阶段项目管理：任务状态会变化，需要根据最新进展更新上下文。
   - 复杂数据分析：先看总体统计，再按异常点深入。
   - 安全审计：需要根据发现的风险逐步扩展攻击面和证据链。

   一次性加载所有上下文更适合：

   - 小型、封闭、上下文较短的任务，例如审查一个200行脚本。
   - 需要全局一致性判断的任务，例如比较一份短合同中的条款冲突。
   - 信息总量有限且高度相关的任务，例如根据一页产品需求文档写测试用例。
   - 不允许工具调用或多轮交互的场景，例如一次性离线评测。

   三个任务示例：

   | 任务类型 | 更适合策略 | 原因 |
   | --- | --- | --- |
   | 排查大型Web应用的性能问题 | 渐进式披露 | 需要根据日志、代码、数据库和提交记录逐步定位 |
   | 总结一篇短论文 | 一次性加载 | 论文内容有限，整体阅读更利于把握结构 |
   | 维护持续一周的重构任务 | 渐进式披露 | 任务状态不断变化，需要结合笔记和最新文件状态 |
   | 审查一个单文件算法实现 | 一次性加载 | 文件短，完整上下文能减少遗漏 |
   | 构建竞品调研报告 | 渐进式披露 | 需要根据初步发现继续检索和筛选信息 |

   总结来说，渐进式披露适合开放、复杂、长程、信息量大的任务；一次性加载适合封闭、短小、静态、强整体性的任务。
