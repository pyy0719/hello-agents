...
1. 本章介绍了三种智能体通信协议：MCP、A2A 和 ANP。请分析：
   ...
   **参考答案：**
   ...
   （1）三种协议的设计理念不同，是因为它们解决的问题层级不同。

   | 协议 | 设计理念 | 核心问题 | 典型对象 |
   | --- | --- | --- | --- |
   | MCP | 上下文共享 | 智能体如何标准化访问工具、数据和外部资源 | 工具、数据库、文件、API |
   | A2A | 对话式协作 | 智能体之间如何分工、协商和交换任务结果 | 研究员、撰写员、客服专家等 Agent |
   | ANP | 网络拓扑 | 大规模 Agent 网络中如何发现、路由和管理服务 | 服务注册中心、Agent 网络、调度系统 |

   MCP 强调"上下文共享"，是因为工具调用不只是执行一个函数。智能体调用数据库、文件系统、GitHub 或搜索服务时，需要知道工具能力、参数 schema、返回数据、资源内容和提示模板。MCP 的核心价值是把外部系统包装成标准化上下文接口，使模型能够以统一方式理解和使用外部能力。

   A2A 强调"对话式协作"，是因为 Agent 与 Agent 的关系不是简单函数调用。一个 Agent 可能需要向另一个 Agent 分派任务、等待结果、追问细节、协商截止时间、接收工件并继续处理。它关注的是任务生命周期和协作流程，而不只是"调用一次工具得到结果"。

   ANP 强调"网络拓扑"，是因为当系统中存在大量 Agent 时，核心问题会变成"如何找到合适的 Agent"和"如何在网络中高效路由"。ANP 关注服务发现、能力匹配、负载均衡、节点连接和容错，它更像智能体网络的基础设施层。

   （2）智能客服系统中的协议选择如下：

   | 功能需求 | 推荐协议 | 理由 |
   | --- | --- | --- |
   | 访问客户数据库和订单系统 | MCP | 数据库、订单系统属于外部工具或资源，适合封装成 MCP 工具，如 `query_customer`、`get_order_status` |
   | 多个专业客服智能体协作处理复杂问题 | A2A | 技术客服、售后客服、销售客服之间需要转交任务、补充信息和协同决策 |
   | 支持大规模并发用户请求 | ANP | 需要服务发现、智能路由和负载均衡，根据客服 Agent 的能力、负载和可用性分配请求 |

   一个合理设计是：前端接待 Agent 收到用户问题后，先通过 MCP 查询客户画像和订单状态；如果问题复杂，再通过 A2A 转给专业 Agent；当并发量很大时，通过 ANP 从多个客服 Agent 实例中选择负载最低、能力匹配的服务。

   （3）三种协议完全可以组合使用，而且真实系统往往需要组合使用。
   ...
   接待 Agent
      ↓
   ANP 服务发现：查找合适的专业 Agent
      ↓
   A2A 协作：接待 Agent 与技术/售后/销售 Agent 对话
      ↓
   专业 Agent
      ↓
   MCP 工具调用：查询 CRM、订单系统、知识库、工单系统
      ↓
   返回处理结果
      ↓
   接待 Agent 整理回复并创建/更新工单
   ```
   ...
   - MCP 负责连接外部系统，把数据库、知识库、订单系统封装为标准工具。
   - A2A 负责专业 Agent 之间的任务协作和结果交换。
   - ANP 负责在大量 Agent 中发现合适服务，并根据能力、负载、成本进行路由。
...
2. MCP（Model Context Protocol）是智能体与工具通信的标准协议。基于 10.2 节的内容，请深入思考：
   ...
   **参考答案：**
   ...
   （1）可以设计一个"数据分析 MCP 服务器"，包含三个工具：

   | 工具 | 输入 | 输出 | 作用 |
   | --- | --- | --- | --- |
   | `query_database` | SQL 或自然语言查询条件 | 表格数据或 JSON 数据 | 从数据库提取原始数据 |
   | `create_visualization` | 数据、图表类型、字段映射 | 图表文件路径或 base64 图片 | 将查询结果转为可视化图表 |
   | `generate_report` | 数据摘要、图表、分析结论 | Markdown/PDF 报告 | 生成最终分析报告 |
   ...
   def query_database(sql: str) -> str:
       ...
       # 实际项目中这里连接数据库并执行查询
       rows = [
           {"date": "2026-06-01", "sales": 1200},
           {"date": "2026-06-02", "sales": 1500}
       ]
       return json.dumps({"rows": rows}, ensure_ascii=False)

   def create_visualization(data_json: str, chart_type: str = "line") -> str:
       """根据数据生成图表"""
       data = json.loads(data_json)
       # 实际项目中可用 matplotlib / plotly 生成图表
   ...
   def generate_report(title: str, data_summary: str, chart_path: str) -> str:
       """生成 Markdown 分析报告"""
       report = f"""# {title}
   ...
   ## 结论
   销售数据存在阶段性波动，建议结合活动和渠道数据继续分析。
   """
       return report

   analytics_server.add_tool(query_database)
   analytics_server.add_tool(create_visualization)
   analytics_server.add_tool(generate_report)
   ...
   - 数据库工具应该限制为只读查询，避免 Agent 误删或修改数据。
   - 可视化工具应接收结构化数据，而不是依赖自然语言结果。
   - 报表工具应整合数据、图表和结论，形成最终工件。
   - 三个工具之间通过 JSON 传递结构化结果，降低解析错误。

   （2）MCP 的三个核心概念可以这样理解：

   | 概念 | 作用 | 例子 |
   | --- | --- | --- |
   | Tools | 可执行动作 | 查询数据库、发送邮件、读取文件、生成图表 |
   | Resources | 可读取上下文数据 | 文档、代码文件、数据库表 schema、配置说明 |
   | Prompts | 可复用提示模板 | 代码审查模板、数据分析模板、周报生成模板 |

   Resources 的设计目的，是让服务器向客户端暴露可作为上下文的数据。例如代码仓库 MCP 服务器可以暴露 `file://README.md`、`schema://orders`、`doc://api-guide` 等资源。模型不一定要执行动作，只需要读取这些资源来理解背景。

   Prompts 的设计目的，是让服务器提供标准化、可复用的任务模板。例如数据分析服务器可以提供"销售趋势分析"、"异常检测报告"、"经营周报生成"等模板，客户端可以直接调用这些模板来构造高质量提示。

   应用场景：企业数据分析助手。

   ```text
   Resources:
   - schema://sales_orders        订单表结构
   - doc://metric_definition      指标口径说明
   - file://last_month_report.md  上月分析报告

   Prompts:
   - sales_trend_analysis         销售趋势分析模板
   - anomaly_investigation        异常波动排查模板
   ...
   用户：生成本月销售分析报告
      ↓
   Agent 读取 Resources：表结构、指标口径、上月报告
      ↓
   Agent 使用 Prompt：销售趋势分析模板
      ↓
   Agent 调用 Tools：查询数据、生成图表、输出报告
   ```

   这种组合比单纯工具调用更强，因为模型既知道"能做什么"，也知道"基于哪些上下文做"，还知道"按照什么任务模板做"。

   （3）JSON-RPC 2.0 + stdio 的优势：

   - 协议简单，JSON 格式易读、易调试。
   - 与编程语言无关，Python、Node.js、Go 等都可以实现。
   - stdio 适合本地进程间通信，不需要额外开放端口。
   - 安全边界相对清楚，客户端可以启动和管理 MCP 子进程。
   - 很适合 IDE、本地 Agent、桌面应用调用本地工具。

   局限性：

   - stdio 更适合本地通信，不适合远程服务。
   - 不天然支持多客户端并发访问。
   - 进程生命周期管理复杂，子进程崩溃需要重启。
   - 对流式响应、长任务、认证、限流等生产能力支持不足。
   - 网络部署、服务发现和负载均衡需要额外设计。

   如果要支持远程 MCP 服务器，可以扩展传输层，而不是改变 MCP 的工具语义：
   ...
   class HTTPTransport:
       ...
       async def send_request(self, method, params):
           ...
           return await http_post(self.base_url, json=payload, headers=headers)
   ```
...
3. A2A（Agent-to-Agent Protocol）支持智能体间的对话式协作。基于 10.3 节的内容，请完成以下扩展实践：
   ...
   **参考答案：**
   ...
   （1）加入 Reviewer 后，协作流程可以设计为：

   ```text
   用户给定论文主题
      ↓
   Researcher：检索资料，输出研究要点和参考来源
      ↓
   Writer：根据研究结果生成论文初稿
      ↓
   Reviewer：评审论文质量，给出评分和修改意见
      ↓
   Writer：根据修改意见生成修订稿
      ↓
   Reviewer：再次确认是否通过
   ...
   def review(text: str) -> str:
       article = text.replace("review", "").strip()
       score = 80
       suggestions = []

       if "摘要" not in article:
           score -= 20
           suggestions.append("补充摘要部分")
       if "主要内容" not in article:
           score -= 20
           suggestions.append("补充主体分析")
       if "结论" not in article:
           score -= 20
           suggestions.append("补充结论")
       if len(article) < 300:
           score -= 10
           suggestions.append("内容略短，建议扩展论证")

       approved = score >= 75
       return str({
           "score": score,
           "approved": approved,
           "suggestions": suggestions or ["结构完整，可以通过"]
       })

   threading.Thread(target=lambda: researcher.run(port=5100), daemon=True).start()
   threading.Thread(target=lambda: writer.run(port=5101), daemon=True).start()
   threading.Thread(target=lambda: reviewer.run(port=5102), daemon=True).start()
   time.sleep(2)

   researcher_client = A2AClient("http://localhost:5100")
   writer_client = A2AClient("http://localhost:5101")
   reviewer_client = A2AClient("http://localhost:5102")

   def create_paper(topic: str):
       research_result = researcher_client.execute_skill("research", f"research {topic}")
       draft = writer_client.execute_skill("write", f"write {research_result.get('result', '')}")
       review_result = reviewer_client.execute_skill("review", f"review {draft.get('result', '')}")

       print("研究结果:", research_result.get("result"))
       print("论文初稿:", draft.get("result"))
       print("审稿意见:", review_result.get("result"))
   ...
   if __name__ == "__main__":
       create_paper("多智能体协作系统")
   ```

   实际项目中可以进一步增加"修订"技能，让 Writer 根据 Reviewer 的建议自动生成第二版论文。

   （2）冲突解决机制可以分为四层：

   | 层级 | 机制 | 说明 |
   | --- | --- | --- |
   | 事实校验 | evidence_request | 要求各 Agent 提供证据来源 |
   | 协商 | negotiation | 各 Agent 提出方案、理由和让步条件 |
   | 投票 | voting | 多个 Agent 对候选方案投票 |
   | 仲裁 | arbitration | 由更高权限 Agent 或人类裁决 |
   ...
   ```json
   {
     "type": "voting",
     "task_id": "task_001",
     "candidates": ["方案A", "方案B", "人工复核"],
     "votes": [
       {"agent": "agent_a", "choice": "方案A", "weight": 0.8},
       {"agent": "agent_b", "choice": "方案B", "weight": 0.7},
       {"agent": "reviewer", "choice": "人工复核", "weight": 1.0}
     ]
   }
   ```
   ...
   关键原则是：不能只比较"谁说得更像真的"，而要比较证据质量、历史可信度、任务风险和置信度。

   （3）A2A 与 AutoGen、CAMEL 的关系：

   | 对象 | 定位 | 是否是协议 | 作用 |
   | --- | --- | --- | --- |
   | A2A | 通信协议 | 是 | 规定 Agent 之间如何传递任务、消息和结果 |
   | AutoGen | 多 Agent 应用框架 | 否 | 提供 Agent 编排、对话管理、工具调用等能力 |
   | CAMEL | 多 Agent 角色扮演/协作框架 | 否 | 通过角色设定和对话模拟实现协作 |

   它们不能简单互相替代。A2A 更像"通信标准"，AutoGen/CAMEL 更像"应用框架"。A2A 可以作为不同框架之间的互操作层。

   让 A2A Agent 与 AutoGen Agent 通信，可以设计一个适配器：
   ...
   AutoGen 执行结果
      ↓
   Adapter 转成 A2A task_result
   ```

   适配器职责：

   - 将 A2A 的 `task` 消息转换成 AutoGen 的用户消息。
   - 调用 AutoGen Agent 执行任务。
   - 将 AutoGen 的回复包装成 A2A 的 `task_result`。
   - 维护任务 ID、会话 ID 和状态映射。
   ...
   class AutoGenA2AAdapter:
       ...
       def handle_a2a_task(self, task_message):
           task_id = task_message["task_id"]
           content = task_message["content"]

           autogen_response = self.autogen_agent.generate_reply(
           ...
           return {
               ...
               "task_id": task_id,
               "status": "completed",
               "result": autogen_response
           }
   ```

   这样，AutoGen 内部仍然使用自己的编排机制，对外则通过 A2A 标准协议暴露能力。
...
4. ANP（Agent Network Protocol）支持大规模智能体网络。基于 10.4 节的内容，请深入分析：
   ...
   **参考答案：**
   ...
   | 拓扑 | 特点 | 适合场景 | 风险 |
   | --- | --- | --- | --- |
   | 星型 | 所有节点连接中心调度器 | 小规模系统、统一调度、便于监控 | 中心节点容易成为瓶颈和单点故障 |
   | 网状 | 节点之间可以直接互连 | 高自治、多 Agent 协商、去中心化协作 | 连接复杂，路由和权限管理成本高 |
   | 分层 | 上层调度，下层执行，按领域分组 | 中大型系统、企业组织、多业务域系统 | 层级设计不合理会造成信息延迟 |
   | 混合型 | 分层管理 + 局部网状连接 | 大规模生产系统 | 实现复杂，但可扩展性最好 |

   规模演进建议：

   ```text
   10 个 Agent：
   星型拓扑即可，一个协调者管理所有 Agent。

   100 个 Agent：
   使用分层拓扑，按能力划分为搜索、代码、数据、客服等子域。

   1000 个 Agent：
   使用混合拓扑。全局服务发现 + 区域调度器 + 局部网状协作。
   ```
   ...
   核心原则是：规模越大，越不能依赖单一中心节点；需要通过分区、分层、缓存和局部自治降低全局调度压力。

   （2）智能路由算法可以综合考虑能力匹配、负载、成本、可信度和网络延迟。
   ...
   - `capability_match`：任务需要的能力与 Agent 能力的匹配程度。
   - `load`：当前负载，越低越好。
   - `trust_score`：历史表现和安全评分。
   - `resource_match`：CPU、内存、GPU 等资源是否满足任务。
   - `latency_score`：网络延迟越低越好。
   - `cost_penalty`：调用成本越高扣分越多。
   ...
   ```python
   def route_task(task, discovery, network):
       required_type = task["type"]
       required_caps = set(task["capabilities"])

       candidates = discovery.discover_services(service_type=required_type)

       scored = []
       for service in candidates:
           caps = set(service.capabilities)
           capability_match = len(required_caps & caps) / max(len(required_caps), 1)
           if capability_match < 0.8:
               continue

           load = service.metadata.get("load", 1.0)
           trust = service.metadata.get("trust", 0.5)
           latency = service.metadata.get("latency_ms", 100)
           latency_score = max(0, 1 - latency / 1000)
           cost = service.metadata.get("price", 0.0)
   ...
   在复杂网络中，还可以先选目标 Agent，再用 Dijkstra 或 A* 根据延迟、带宽、可靠性计算最优路径。

   （3）智能城市中的关键 Agent 故障，需要完整容错机制。

   故障检测：

   - 心跳检测：定期向交通管理 Agent 发送健康检查。
   - 超时检测：调用超过阈值未响应则标记异常。
   - 结果异常检测：返回数据格式错误、决策明显异常时触发告警。
   - 多点交叉验证：通过道路传感器、摄像头 Agent、地图 Agent 验证交通 Agent 输出。
   ...
   主交通 Agent 故障
      ↓
   ANPDiscovery 将其状态标记为 unhealthy
      ↓
   路由器选择备用交通 Agent
      ↓
   将新请求转发到备用节点
   ...
   - 定期将交通 Agent 的关键状态写入共享状态存储。
   - 状态包括路口拥堵指数、信号灯策略、事故事件、正在执行的调度计划。
   - 备用 Agent 启动时从最近 checkpoint 恢复。
   - 恢复后先进入只读观察模式，确认状态一致后再接管控制。
   ...
   - 如果主备都不可用，启用规则系统，如固定红绿灯策略。
   - 对高风险控制动作要求人工确认。
   - 暂停非关键优化任务，优先保证安全和基本通行。

   容错机制的关键不是"永远不出错"，而是出错后能快速发现、隔离、切换和恢复。
...
5. 智能体通信协议的安全性和隐私保护是实际应用中的关键问题。请思考：
   ...
   **参考答案：**
   ...
   （1）MCP 工具无限制调用的安全风险包括：

   - 危险工具误调用：Agent 可能误调用删除文件、执行命令、修改数据库等工具。
   - Prompt Injection：外部文档诱导模型调用敏感工具。
   - 数据泄露：读取 `.env`、密钥、客户数据后发送给外部服务。
   - 工具组合攻击：单个工具看似安全，多个工具组合后可完成数据窃取。
   - 权限过大：所有 Agent 默认拥有 MCP 服务器全部工具权限。
   - 审计缺失：无法追踪谁在什么时候调用了什么工具。

   权限控制机制可以采用"工具分级 + 显式授权 + 审计追踪"。

   工具风险分级：

   | 等级 | 操作示例 | 策略 |
   | --- | --- | --- |
   | 低风险 | 读取公开文档、查询帮助信息 | 可自动调用 |
   | 中风险 | 查询内部数据、运行测试、读取普通项目文件 | 需要角色权限或会话授权 |
   | 高风险 | 写文件、修改数据库、发送邮件、创建工单 | 调用前要求用户确认 |
   | 禁止 | 删除关键文件、读取密钥、执行任意 shell | 默认拒绝或只允许管理员 |
   ...
   ```json
   {
     ...
     "allowed_tools": ["query_order", "query_customer_profile"],
     "denied_tools": ["delete_file", "run_shell"],
     "requires_approval": ["refund_order", "update_customer_info"]
   }
   ```
   ...
   Agent 计划调用工具
      ↓
   权限管理器检查 tool_name、参数和调用上下文
      ↓
   低风险：直接执行
      ↓
   中风险：检查角色权限
      ↓
   高风险：展示调用原因和影响范围，请用户审批
      ↓
   执行后写入审计日志
   ```

   对危险操作，还应加入参数级校验。例如允许读取 `docs/`，但禁止读取 `.env`；允许执行 `pytest`，但禁止执行 `rm -rf`。

   （2）端到端加密方案可以结合身份认证、密钥协商、消息加密和签名校验。

   基础组件：

   - 每个 Agent 拥有唯一身份 ID，例如 DID 或证书身份。
   - 每个 Agent 拥有公私钥对。
   - 公钥通过注册中心或 DID 文档发布。
   - 通信时使用接收方公钥加密会话密钥。
   - 消息体使用对称加密算法加密。
   ...
   用 A 的私钥对消息摘要签名
      ↓
   发送 encrypted_payload + encrypted_key + signature
   ```
   ...
   ```json
   {
     "from": "did:agent:customer_support",
     "to": "did:agent:order_service",
   ...
   访问控制方面，可以使用 RBAC 或 ABAC：

   - RBAC：按角色授权，如客服 Agent 只能查询订单，不能退款。
   - ABAC：按属性授权，如只能访问当前用户授权范围内的数据。
   - 最小权限原则：每个 Agent 只拥有完成任务所需的最小权限。

   （3）信任评估系统可以为每个 Agent 维护动态信任分。
   ...
   | --- | --- |
   | 历史成功率 | 任务完成率、错误率、超时率 |
   | 协作质量 | 返回结果是否有用、是否按格式输出 |
   | 安全行为 | 是否尝试越权访问、是否发送异常请求 |
   | 社区评价 | 其他 Agent 或用户的评分 |
   | 可验证性 | 输出是否提供证据、来源是否可信 |
   | 稳定性 | 服务可用率、响应延迟、负载表现 |
   ...
   - 出现越权访问、恶意请求，信任分大幅下降。
   - 长时间稳定运行，信任分缓慢恢复。
   - 高风险任务只允许高信任 Agent 参与。

   通信策略：

   | 信任等级 | 策略 |
   | --- | --- |
   | 高信任 | 可处理重要任务，允许访问更多资源 |
   | 中信任 | 可处理普通任务，敏感操作需要审批 |
   | 低信任 | 只能访问公开资源，输出需要交叉验证 |
   | 黑名单 | 拒绝通信或隔离 |
   ...
   在大规模网络中，信任系统不能只看单次表现，而要结合长期行为、异常检测和多方评价。同时要防止刷分和恶意互评，可以引入可信审计节点或权重更高的人工反馈。
