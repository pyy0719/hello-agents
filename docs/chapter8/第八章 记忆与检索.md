...
class EpisodicMemory:
    ...
    def __init__(self, config: MemoryConfig):
        self.doc_store = SQLiteDocumentStore(config.database_path) # 创建 SQLite 文档存储
        self.vector_store = QdrantVectorStore(config.qdrant_url, config.qdrant_api_key) # 创建 Qdrant 向量存储
        self.embedder = create_embedding_model_with_fallback() # 创建 embedding 模型 把文本变成向量
        self.sessions = {}  # 会话索引
...
1. 本章介绍了四种记忆类型：工作记忆、情景记忆、语义记忆和感知记忆。请分析：
   ...
   **参考答案：**
   ...
   情景记忆更强调时间近因性，是因为它记录的是"发生过的事件"。事件通常有明显的时间价值，比如用户刚刚问过的问题、最近一次训练记录、最近一次报错排查，这些信息越新，越可能对当前决策有帮助。因此情景记忆的评分会把语义相似度和时间近因性结合起来。语义记忆更强调图检索，是因为它记录的是相对稳定的知识、概念和关系，例如"Python用于机器学习"、"张三擅长React"。这类知识不一定越新越重要，关键在于实体之间的关系是否能帮助推理，所以会引入Neo4j图检索。

   如果设计个人健康管理助手，可以这样组合四种记忆：工作记忆保存当前对话中的临时上下文，例如用户刚刚输入的早餐、今天的运动计划、当前正在计算的热量；情景记忆保存具体事件，例如"2026年6月8日用户跑步30分钟"、"昨晚睡眠6小时"；语义记忆保存长期稳定的健康知识和用户画像，例如"用户乳糖不耐受"、"高蛋白饮食有助于增肌"；感知记忆保存图片、音频、设备数据等多模态信息，例如食物照片、体检报告截图、智能手环睡眠曲线。

   工作记忆应该在"短期信息反复出现、重要性较高、对未来长期有价值"时整合为长期记忆。可以设计自动触发条件：同一信息在多轮对话中被反复提到；重要性评分超过阈值；用户明确说"记住这个"；某条工作记忆被频繁检索；或者它和已有长期记忆高度相关。满足条件后，可以把它转为情景记忆或语义记忆，例如把"用户最近连续三天睡眠不足"整合为情景记忆，把"用户通常晚上12点后入睡"整合为语义记忆。
...
2. 在8.3节的RAG系统中，我们使用MarkItDown将各种格式文档统一转换为Markdown。请深入思考：
   ...
   **参考答案：**
   ...
   对没有明确标题结构的文档，不能只依赖Markdown标题分块。小说可以按自然段、章节标记、场景转换、人物对话边界分块；法律条文可以按条、款、项、目分块，并保留法条编号作为元数据。一个基于语义边界的分块算法可以先按段落切分，再计算相邻段落的语义相似度，如果相似度突然下降，就认为出现了新的主题边界；同时控制每个块的token数量，并加入一定重叠，避免上下文断裂。

   以技术文档问答为例，基础检索适合用户提问和文档表述非常接近的情况，例如用户问"RAG是什么"，文档里也有"RAG是一种..."。MQE适合用户问题表述比较口语化或有多种说法的情况，例如"怎么让模型查资料再回答"可以扩展为"检索增强生成"、"RAG工作流程"、"知识库问答"。HyDE适合问题和答案表达差异较大的情况，它先生成一段可能的答案，再用答案式文本去检索真实文档，适合专业领域和抽象问题。

   三种嵌入方案各有取舍。百炼API准确性较好，部署简单，适合学习和线上原型，但需要联网并按调用付费；本地Transformer可以离线部署，隐私更好，长期成本可控，但需要本地算力和模型管理；TF-IDF速度快、成本低、实现简单，但主要依赖关键词匹配，语义理解能力弱。选型建议是：学习阶段优先用百炼API；隐私敏感或内网部署用本地Transformer；只做简单关键词检索或兜底方案时使用TF-IDF。
...
3. 记忆系统的"遗忘"机制是模拟人类认知的重要设计。基于8.2.3节的MemoryTool，请完成以下扩展实践：
   ...
   **参考答案：**
   ...
   智能遗忘策略可以为每条记忆计算一个保留分数，例如：保留分数 = 重要性 × 0.4 + 访问频率 × 0.3 + 时间新近性 × 0.2 + 用户标记 × 0.1。分数低于阈值的记忆可以被遗忘，分数较低但仍可能有价值的记忆可以先归档。这样比单纯按时间或容量删除更合理，因为有些旧记忆虽然时间久，但非常重要或经常被访问。

   记忆归档可以把长期不用但仍有价值的记忆从热存储转移到冷存储，例如从Qdrant活跃集合转移到归档集合，或从Neo4j活跃图转移到归档标签节点。工作记忆通常不归档，过期后删除或整合；情景记忆适合按时间归档，例如半年前的学习事件；语义记忆适合按重要性和访问频率归档，重要知识保留在热存储；感知记忆中的大文件可以把原始文件转移到对象存储，只保留摘要和索引。

   对敏感信息来说，仅仅从一个数据库删除通常不够。因为同一信息可能同时存在于SQLite、Qdrant向量、Neo4j实体关系、缓存、日志、备份文件中。彻底删除应包括：删除原始文本记录；删除对应向量点；删除图数据库中的实体、关系或与该用户相关的边；清理缓存和日志；标记备份中的删除请求；必要时重建索引。对于向量数据库，还要通过memory_id、user_id等元数据定位相关向量，不能只删原文。
...
4. 在8.4节的"智能学习助手"案例中，我们结合了MemoryTool和RAGTool。请深入分析：
   ...
   **参考答案：**
   ...
   RAG适合回答"文档里有什么"的问题，例如解释PDF中的概念、查找某一章节内容、引用技术资料。Memory适合回答"用户过去做过什么、学过什么、偏好什么"的问题，例如回顾学习历史、根据用户薄弱点推荐内容、记住用户的笔记。智能路由可以先判断问题意图：如果问题包含"文档、这一章、资料中、根据PDF"等信号，优先走RAG；如果包含"我之前、我学过、我的笔记、上次"等信号，优先走Memory；如果两类都相关，就同时检索，再把结果合并给LLM生成答案。

   更智能的学习报告可以从情景记忆中提取用户的学习时间线，从工作记忆中获取当前学习目标，从语义记忆中整理用户已经掌握的概念，从RAG检索中对照课程知识结构，找出未覆盖或频繁提问但仍不稳定的知识点。报告可以包括：学习轨迹、已掌握概念、高频问题、薄弱知识点、推荐复习章节、下一步学习计划。检索策略上可以结合RAG搜索课程内容，Memory搜索用户历史，再用LLM总结。

   多用户部署时，Qdrant可以通过metadata中的`user_id`、`rag_namespace`进行过滤，也可以为不同用户或租户建立不同collection；小规模系统用metadata过滤更简单，大规模或强隔离系统可以按租户分collection。Neo4j中可以给节点和关系都加`user_id`属性，查询时强制带上用户过滤条件，也可以按用户或租户使用不同database。性能优化方面，可以为`user_id`、`rag_namespace`、`memory_type`建立索引，限制检索范围，使用分页和top-k，避免跨用户全库扫描。
...
5. 语义记忆使用了Neo4j图数据库来存储知识图谱。请思考：
   ...
   **参考答案：**
   ...
   自动抽取实体和关系的准确性取决于文本质量、领域复杂度、抽取模型能力和规则设计。简单句子通常较准，例如"张三是Python开发者"；复杂句、代词、省略、嵌套关系、专业术语和反事实表达容易出错。例如"张三不再负责React项目"如果抽成"张三负责React项目"就是错误关系。知识图谱质量评估可以从实体准确率、关系准确率、重复实体比例、孤立节点比例、人工抽样审核通过率、与已有知识冲突数量等指标入手，也可以给每条关系加入置信度。

   一个适合图查询的场景是学习路径推荐。假设图中有"概念-依赖-概念"关系，例如"Transformer依赖Attention"、"Attention依赖矩阵乘法"、"矩阵乘法依赖线性代数"。当用户问"我想学Transformer，但我缺哪些前置知识？"时，Neo4j可以做多跳路径查询，找出Transformer向前依赖的所有概念，并结合用户已掌握概念过滤，得到需要补的知识路径。纯向量检索只能找语义相似文本，很难稳定做这种依赖链推理。

   混合策略比纯向量检索更适合关系型、路径型、约束型问题。比如"谁和React、Vue都有关联？"、"从A概念到B概念中间经过哪些知识点？"、"用户学过的内容和机器学习之间有什么联系？"这类问题需要实体关系和多跳推理，图检索优势明显。纯向量检索适合找语义相似片段，但不擅长保证关系精确性；向量检索负责召回语义相关内容，图检索负责补充结构化关系，两者结合更稳。
