...
1. 本章介绍了三种经典的智能体范式:`ReAct`、`Plan-and-Solve` 和 `Reflection`。请分析:
   ...
   **参考答案：**
   ...
   三种范式的核心区别在于：它们组织“思考”和“行动”的节奏不同。

   `ReAct` 是“边想边做”。智能体每一轮先产生 `Thought`，再选择一个 `Action` 调用工具，然后根据 `Observation` 继续下一轮判断。它适合信息不确定、需要外部反馈的任务，例如搜索实时信息、查询数据库、调用业务 API。

   `Plan-and-Solve` 是“先规划，再执行”。智能体先把复杂任务拆成多个步骤，再按照计划逐步完成。它适合目标清晰、步骤相对稳定的任务，例如数学应用题、旅行计划、报告生成流程。

   `Reflection` 是“先生成，再检查和优化”。智能体先给出一个初始结果，再通过反思发现问题并迭代改进。它适合对质量要求较高的任务，例如代码生成、文章润色、方案评审。

   如果设计“智能家居控制助手”，我会以 `ReAct` 作为基础架构。因为智能家居助手需要频繁和外部设备交互，例如查询当前室温、打开空调、调节灯光、关闭窗帘、确认设备状态。这些都是典型的“行动-观察-再行动”流程。

   例如用户说：“我有点热，帮我调舒服一点。”智能体需要先查询室温和用户习惯，再调用空调工具调温，之后可能还要根据反馈调整风速或窗帘状态。这种场景更适合 `ReAct`。

   三种范式也可以组合使用。一个更完整的智能家居智能体可以这样设计：

   - `Plan-and-Solve` 负责高层任务拆解，例如把“准备睡眠环境”拆成关灯、调空调、拉窗帘、开启安防。
   - `ReAct` 负责每个步骤中的设备调用和状态确认。
   - `Reflection` 负责根据用户后续反馈优化策略，例如发现用户经常手动调高温度，就更新默认睡眠温度。

   面试中可以总结为：`Plan-and-Solve` 负责“拆任务”，`ReAct` 负责“调用工具执行任务”，`Reflection` 负责“检查和优化结果”。真实系统里三者不是互斥的，而是可以组合成一个更完整的 Agent Loop。
...
2. 在4.2节的 `ReAct` 实现中，我们使用了正则表达式来解析大语言模型的输出（如 `Thought` 和 `Action`）。请思考:
   ...
   **参考答案：**
   ...
   当前正则解析的主要问题是过于依赖模型严格遵守格式。只要模型输出稍微偏离模板，解析就可能失败。

   可能失败的情况包括：

   - 模型漏掉 `Action:` 前缀，只输出 `Search[xxx]`。
   - 模型把 `Thought` 和 `Action` 写成中文，例如“思考：”“行动：”。
   - 模型输出多个 `Action`，正则不知道应该执行哪一个。
   - 模型在 `Action` 参数中输出了多余换行、括号或解释文字。
   - 模型直接给出最终答案，没有按照 `Finish[answer]` 格式输出。
   - 工具参数里本身包含 `]`、`,` 等特殊字符，导致正则截断错误。

   更鲁棒的方案有：

   - **JSON 输出**：要求模型固定输出 `{"thought": "...", "action": "...", "action_input": "..."}`。
   - **函数调用 / Tool Calling**：使用模型厂商提供的工具调用协议，让模型直接返回结构化工具名和参数。
   - **Pydantic / JSON Schema 校验**：对模型输出做结构校验，失败后要求模型重试。
   - **有限状态机解析**：把智能体流程设计成明确状态，例如 `thinking`、`acting`、`finished`，减少自由文本解析。
   - **输出修复器**：如果第一次输出格式错误，再调用一次模型，让它把错误输出修复成合法结构。

   如果用 JSON 格式，可以把提示词改成要求模型只输出如下结构：
   ...
   代码上可以用 `json.loads()` 解析，然后检查 `action` 是否在工具列表中。

   两种方案对比如下：

   | 方案 | 优点 | 缺点 |
   |---|---|---|
   | 正则解析 | 实现简单，适合教学和原型验证 | 对格式非常敏感，生产稳定性较差 |
   | JSON 解析 | 结构清晰，容易校验，方便扩展参数 | 模型仍可能输出非法 JSON，需要重试机制 |
   | Tool Calling | 最稳定，工具名和参数天然结构化 | 依赖模型 API 能力，跨模型适配成本更高 |

   生产环境中更推荐使用 Tool Calling 或 JSON Schema，而不是完全依赖正则表达式。
...
3. 工具调用是现代智能体的核心能力之一。基于4.2.2节的 `ToolExecutor` 设计，请完成以下扩展实践:
   ...
   **参考答案：**
   ...
   可以为 `ToolExecutor` 增加一个 `Calculator` 工具。它的作用是接收数学表达式，返回计算结果。为了安全，不能直接对用户输入使用不受限制的 `eval()`，否则可能执行危险代码。更稳妥的做法是只允许数字、括号和基础运算符。
   ...
   tool_executor.register_tool(
       name="Calculator",
       func=calculator,
       description="用于计算数学表达式，例如 Calculator[(123 + 456) * 789 / 12]"
   )
   ```

   对于工具选择失败，可以设计一个失败反馈机制：

   - 记录连续失败次数，例如 `failed_tool_count`。
   - 当工具不存在时，返回可用工具列表，让模型重新选择。
   - 当参数格式错误时，返回该工具的参数示例。
   - 当连续失败超过阈值时，强制模型进入“重新思考”阶段，不允许继续盲目调用同一个工具。

   例如观察结果可以设计成：

   ```text
   Observation: 工具调用失败。原因：Calculator 参数必须是数学表达式。
   可用示例：Calculator[(123 + 456) * 789 / 12]
   请重新选择工具或修正参数。
   ```

   如果工具数量增加到 50 个甚至 100 个，当前把所有工具描述一次性塞进 Prompt 的方式就不太合适了。问题主要有三个：

   - Prompt 太长，占用上下文窗口。
   - 模型容易在大量工具中选错。
   - 工具描述维护成本高，新增和下线工具都不方便。

   工程上可以这样优化：

   - 按业务领域给工具分组，例如搜索类、计算类、订单类、邮件类。
   - 增加工具检索器，先根据用户问题召回最相关的 3 到 5 个工具，再交给模型选择。
   - 为工具建立元数据，包括名称、功能、输入参数、输出格式、适用场景、失败示例。
   - 使用向量检索或关键词检索，从工具库中动态选择候选工具。
   - 对高风险工具增加权限控制和人工确认，例如支付、退款、发邮件。

   所以工具数量变多后，Agent 不应该“看到所有工具”，而应该先经过一个工具路由或工具检索模块，只把当前任务最相关的工具暴露给模型。
...
4. `Plan-and-Solve` 范式将任务分解为"规划"和"执行"两个阶段。请深入分析:
   ...
   **参考答案：**
   ...
   静态计划的问题在于：现实任务经常会出现变化。比如某个航班售罄、酒店无房、租车价格超预算，此时原计划就不能继续机械执行。

   动态重规划机制可以这样设计：

   - 执行每一步后判断结果状态，例如 `success`、`failed`、`partial_success`。
   - 如果失败，记录失败原因和当前上下文。
   - 调用规划器重新生成后续计划，而不是从头开始。
   - 新计划需要保留已经成功完成的步骤，只修改失败步骤之后的部分。
   - 设置最大重规划次数，避免无限循环。
   ...
   生成初始计划 -> 执行第 i 步 -> 检查结果
   如果成功：继续第 i+1 步
   如果失败：把失败原因、已完成步骤、原始目标交给 Planner，生成修订计划
   ```

   对于“预订一次从北京到上海的商务旅行”这个任务，我会优先选择 `ReAct + Plan-and-Solve` 的组合，而不是单独使用其中一种。

   原因是这个任务既需要规划，也需要动态工具调用。机票、酒店、租车之间有依赖关系，适合先用 `Plan-and-Solve` 制定整体计划；但预订过程中会遇到实时价格、库存、时间冲突、支付确认等问题，这些又需要 `ReAct` 根据外部工具反馈动态调整。

   更合理的设计是：

   - `Plan-and-Solve` 先生成高层计划：查航班、订酒店、安排租车、确认行程、发送通知。
   - `ReAct` 在每一步中调用具体工具，例如航班查询 API、酒店 API、租车 API、日历 API。
   - 如果某一步失败，触发动态重规划。

   分层规划系统可以分为两层：

   - 高层计划：描述大的任务阶段，例如“确定出行时间”“预订交通”“预订住宿”“生成行程单”。
   - 子计划：针对每个阶段生成可执行步骤，例如“查询航班”“比较价格”“检查是否符合预算”“提交预订”。

   分层规划的优势是：

   - 复杂任务更清晰，不会一次性生成过长计划。
   - 每个子任务可以独立执行和重规划。
   - 更容易复用，例如“预订酒店”可以作为通用子流程。
   - 更适合多人协作或多 Agent 系统，不同 Agent 可以负责不同子任务。

   面试中可以总结为：静态计划适合简单稳定任务，真实业务更需要“计划执行监控重规划”的闭环。
...
5. `Reflection` 机制通过"执行-反思-优化"循环来提升输出质量。请思考:
   ...
   **参考答案：**
   ...
   使用两个不同模型是很常见的工程优化思路。可以让较快、较便宜的模型负责初始执行，让更强的模型负责反思和评审。

   这样做的好处是：

   - 降低整体成本，不必每一步都调用最强模型。
   - 提升反思质量，因为强模型更容易发现逻辑漏洞、边界问题和优化空间。
   - 执行速度更快，适合需要多轮迭代的任务。

   可能的问题是：

   - 两个模型风格不同，执行模型可能无法完全理解反思模型的建议。
   - 强模型提出的优化建议可能超出弱模型实现能力。
   - 系统复杂度上升，需要处理模型之间的上下文传递和格式对齐。

   当前用“反馈中包含 `无需改进`”作为终止条件比较简单，适合教学演示，但生产中不够稳定。因为模型可能换一种说法，例如“当前方案已经足够好”，代码就无法识别。

   更智能的终止条件可以结合多个信号：

   - 反思模型输出结构化评分，例如正确性、效率、可读性都达到 8 分以上。
   - 自动测试全部通过。
   - 连续两轮没有发现新的实质性问题。
   - 优化前后结果差异很小。
   - 达到成本、时间或迭代次数上限。
   ...
   ```json
   {
     ...
     "score": 9,
     "issues": [],
     "reason": "代码已经使用筛法，复杂度合理，边界情况已处理。"
   }
   ```

   对于“学术论文写作助手”，可以设计多维度 Reflection：

   - **段落逻辑性反思**：检查段落之间是否有清晰递进关系，是否存在跳跃、重复或论证断裂。
   - **方法创新性反思**：检查论文是否说明了相对已有工作的改进点，创新点是否具体。
   - **实验设计反思**：检查实验是否有对照组、评价指标是否合理、结果是否支持结论。
   - **语言表达反思**：检查表达是否学术、准确、简洁，是否存在口语化表述。
   - **引用规范反思**：检查关键论断是否有引用，引用格式是否统一，是否存在缺失引用。
   - **摘要和结论反思**：检查摘要是否覆盖问题、方法、结果和贡献，结论是否夸大。

   系统流程可以是：

   ```text
   生成初稿 -> 逻辑性评审 -> 方法创新性评审 -> 实验与引用评审 -> 语言润色 -> 最终一致性检查
   ```

   每个反思维度可以由不同提示词或不同评审 Agent 完成，最后由一个汇总模块整合修改建议。
...
6. 提示词工程是影响智能体最终效果的关键技术。本章展示了多个精心设计的提示词模板。请分析:
   ...
   **参考答案：**
   ...
   `ReAct` 提示词的重点是让模型按照 `Thought -> Action -> Observation` 的循环行动。因此它必须明确告诉模型有哪些工具、工具如何调用、什么时候使用 `Finish` 结束。它强调的是动态决策和工具调用。

   `Plan-and-Solve` 提示词的重点是让模型先生成一个完整计划。因此它更强调任务拆解、步骤顺序和输出结构。它不要求模型每一步都马上调用工具，而是先把问题拆成稳定的子任务。

   两者的差异服务于各自的核心逻辑：

   - `ReAct` 的 Prompt 关注“下一步做什么”。
   - `Plan-and-Solve` 的 Prompt 关注“整体应该怎么做”。
   - `ReAct` 需要工具列表和历史记录。
   - `Plan-and-Solve` 需要清晰的计划格式。

   修改角色设定会明显影响模型输出风格。例如：

   ```text
   你是一位注重代码可读性的开源项目维护者。
   请从命名、注释、模块划分、可维护性和用户贡献便利性角度审查代码。
   ```

   相比“极其严格的代码评审专家”，这个角色更可能关注：
   ...
   这说明角色设定会改变模型的关注重点。角色不是装饰，它会影响模型评估问题的视角。

   可以给 `ReAct` 加一个 `few-shot` 示例来提升格式稳定性：

   ```text
   示例：
   Question: 计算 (10 + 5) * 3 的结果
   Thought: 这是一个数学计算问题，我应该使用计算器工具。
   Action: Calculator[(10 + 5) * 3]
   Observation: 45
   Thought: 我已经得到了计算结果，可以回答用户。
   Action: Finish[结果是 45。]
   ```

   加入示例前，模型可能输出自由文本，甚至漏掉 `Action:`。加入示例后，模型更容易模仿固定格式，解析成功率会更高。

   总结来说，提示词不仅是在“告诉模型任务”，更是在定义智能体的行为协议。好的提示词应该同时说明角色、目标、工具、输出格式、失败处理和结束条件。
...
7. 某电商初创公司现在希望使用"客服智能体"来代替真人客服实现降本增效，它需要具备以下功能:
   ...
   **参考答案：**
   ...
   这个客服智能体适合使用混合架构：`Plan-and-Solve + ReAct + Reflection`。

   `Plan-and-Solve` 用来拆解退款处理流程，例如理解诉求、查询订单、判断政策、生成回复、发送邮件。

   `ReAct` 用来调用外部工具，例如订单系统、物流系统、退款政策库、邮件系统。

   `Reflection` 用来处理争议场景。当模型置信度低、退款金额较大、用户情绪强烈或政策边界不清晰时，系统应该先反思自己的判断，再给出更审慎的建议，必要时转人工。

   至少需要以下工具：

   | 工具 | 功能 |
   |---|---|
   | `OrderQueryTool` | 根据用户 ID 或订单号查询订单金额、商品信息、支付状态 |
   | `LogisticsQueryTool` | 查询物流状态，例如未发货、运输中、已签收 |
   | `RefundPolicyTool` | 检索公司退款政策，判断是否符合退款条件 |
   | `RefundActionTool` | 创建退款申请、批准退款或拒绝退款 |
   | `EmailTool` | 根据最终决策向用户发送邮件 |
   | `HumanHandoffTool` | 高风险或低置信度场景转人工客服 |

   提示词设计要同时约束“公司利益”和“用户体验”。可以包含以下原则：

   ```text
   你是一个电商客服智能体。
   你的目标是在遵守公司退款政策的前提下，尽量给用户清晰、友好、可执行的答复。
   你不能编造订单信息，必须通过工具查询。
   你不能绕过退款政策直接承诺退款。
   如果政策不明确、用户情绪强烈、金额较大或你的置信度低于阈值，必须转人工处理。
   回复用户时要礼貌、简洁，并说明处理依据和下一步操作。
   ```

   产品上线后的风险包括：

   - **错误退款或错误拒绝**：可能造成公司损失或用户投诉。可以通过规则校验、金额阈值、人工复核降低风险。
   - **幻觉问题**：模型可能编造政策或订单状态。必须要求它通过工具查询，不允许凭空判断。
   - **越权操作**：模型可能执行高风险操作。需要权限系统和操作审批，例如大额退款必须人工确认。
   - **用户隐私泄露**：客服系统涉及订单、地址、手机号等敏感信息。需要脱敏、权限控制、日志审计。
   - **情绪处理不当**：模型回复可能不够友好。可以增加语气检查模块或 Reflection 机制。
   - **低置信度决策**：边界案例不应该强行自动处理，应该转人工。

   一个较完整的流程可以是：

   ```text
   用户提出退款 -> 理解退款原因 -> 查询订单和物流 -> 检索退款政策
   -> 生成初步决策 -> Reflection 检查风险 -> 高置信度自动处理 / 低置信度转人工
   -> 生成并发送用户回复
   ```

   面试中可以强调：客服智能体不能只追求“能回答”，更重要的是可控、可追踪、可回滚。涉及钱、隐私和用户权益的场景，必须有工具校验、权限控制、日志审计和人工兜底。
