...
class PositionWiseFeedForward(nn.Module):
    ...
    def __init__(self, d_model, d_ff, dropout=0.1):
        ...
        self.dropout = nn.Dropout(dropout) # dropout 是随机丢掉一部分神经元输出，防止过拟合。你现在先把它理解成“训练时加一点噪声，让模型更稳”。
        self.linear2 = nn.Linear(d_ff, d_model)
        self.relu = nn.ReLU() # 清除负数
...
1. 自然语言处理中，语言模型经历了从统计到神经网络的模型演进。
   ...
   - 请使用本章提供的迷你语料库（`datawhale agent learns`, `datawhale agent works`），计算句子 `agent works` 在Bigram模型下的概率
     ...
     你的答案：先算agent的概率=总语料agent的数量/总语料的词数 = 2/6 = 0.333
     再算P(work|agent) = count(agent works)/count(agent) = 1/2 = 0.5
     P(agent works) = 0.333*0.5 = 0.167

     点评：这题答对了，计算过程和结果都正确。表达上如果把 `P(agent works) = P(agent) * P(works|agent)` 明确写出来，会更完整。

     参考答案：在语料库 `datawhale agent learns datawhale agent works` 中，总词数为 6，`agent` 出现 2 次，因此
     `P(agent) = 2 / 6 = 1 / 3`。
     Bigram `agent works` 出现 1 次，而 `agent` 出现 2 次，因此
     `P(works|agent) = 1 / 2`。
     所以句子 `agent works` 在 Bigram 模型下的概率为：
     `P(agent works) = P(agent) * P(works|agent) = (1 / 3) * (1 / 2) = 1 / 6 ≈ 0.167`。

   - N-gram模型的核心假设是马尔可夫假设。请解释这个假设的含义，以及N-gram模型存在哪些根本性局限？

     你的答案：可近似的认为，一个词的出现概率只与它前面有限的n-1个词有关
     数据稀疏性，如果一个词从未在语料库中出现过，它的概率计算出来则为0，即使有数据平滑不能完全解决问题，泛化能力差，每个词是孤立的，无法理解词汇在语义上的相似性

     点评：这题答得比较好，已经抓住了马尔可夫假设和数据稀疏性、泛化差、语义缺失这几个核心点。可以再补一句“难以建模长距离依赖”，会更完整。

     参考答案：马尔可夫假设的含义是：一个词出现的概率，不依赖于完整历史，而只依赖于它前面有限个词。对于 N-gram 模型来说，就是近似认为当前词只与前面 `n-1` 个词有关。
     N-gram 模型的根本性局限主要有：
     第一，数据稀疏性严重，很多词组在训练语料中根本没出现过，概率会变成 0。
     第二，难以处理长距离依赖，因为上下文窗口是固定的。
     第三，泛化能力差，模型难以把见过的模式迁移到相似但未见过的表达上。
     第四，词是离散且孤立的，无法自然表达词与词之间的语义相似性。

   - 神经网络语言模型（RNN/LSTM）和Transformer分别是如何克服N-gram模型局限的？它们各自的优势是什么？

     你的答案：开始用连续的向量来表示词，克服了词之间孤立的问题，RNN/LSTM的优势是打破了固定窗口的限制，可以获得历史状态，缺点是序列过长的时候，模型在反向传递的计算过程中会多次连乘，会导致梯度消失或者梯度爆炸，LSTM引入了细胞状态和门控机制来解决长期依赖问题，可以更有效的控制每一步信息的传递，RNN/LSTM无法并行计算，每个节点依赖上一个节点，transformer引入自注意力机制，可以做到并行计算，缺点是实现复杂

     点评：整体方向是对的，而且已经把 RNN/LSTM 和 Transformer 的关键差异说出来了。这题如果分段更清楚一点，会更像标准作答。

     参考答案：神经网络语言模型首先用连续向量表示词，而不是把每个词看成彼此孤立的符号，因此能够学习词之间的语义相似性。RNN 通过隐藏状态把历史信息传递下去，突破了 N-gram 固定窗口的限制；LSTM 进一步通过门控机制和细胞状态缓解梯度消失问题，更擅长建模较长依赖。它们的优势是天然适合序列建模，能利用上下文历史。
     Transformer 则通过自注意力机制直接计算一个词与序列中所有词的关系，不再依赖逐步传递历史状态，因此既更容易捕捉长距离依赖，也更适合并行计算。它的优势是并行性强、长距离建模能力强、扩展性好，这也是它成为现代大语言模型基础架构的重要原因。
...
2. Transformer架构<sup>[4]</sup>是现代大语言模型的基础。其中：
   ...
   - 自注意力机制（Self-Attention）的核心思想是什么？
     ...
     你的答案：处理序列的每一个词的时候，兼顾句子中的所有其他词，并为这些词分配不同的注意力权重，权重越高代表与当前词的关联性越强

     点评：这题答得是对的，而且已经说到本质了。再补一句“最后会用这些权重对所有词的 `V` 向量做加权求和，得到当前词的新表示”，会更完整。

     参考答案：自注意力机制的核心思想是：在处理序列中的某个词时，不只看固定窗口内的邻近词，而是同时考虑整个序列中的所有词，并根据它们与当前词的相关性分配不同的注意力权重。然后利用这些权重对所有词的 `V` 向量进行加权求和，得到当前词融合上下文后的新表示。

   - 为什么Transformer能够并行处理序列，而RNN必须串行处理？位置编码（Positional Encoding）在其中起什么作用？

     你的答案：因为RNN序列中每一次词需要依赖上一个词传递过来的历史信息，transformer单独计算每一个词和其他词的注意力权重，计算注意力的时候是不关注位置关系的，给一个词加一个位置编码保证独一无二

     点评：主要逻辑是对的。可以把“Transformer 并行”说得更准确一点：它不是一个词一个词往后传隐藏状态，而是整句一起计算注意力矩阵，所以可以并行。

     参考答案：RNN 之所以必须串行处理，是因为第 `t` 个位置的隐藏状态依赖第 `t-1` 个位置传来的状态，所以必须按顺序逐步计算。Transformer 则不同，它通过自注意力机制直接计算序列中所有词两两之间的关系，不依赖逐步传递历史状态，因此整句的注意力分数可以并行计算。
     位置编码的作用是向模型显式注入顺序信息，因为自注意力本身只关心词与词之间的关系，不天然区分“谁在前、谁在后”。加入位置编码后，模型才能感知词元的先后顺序和相对位置。

   - Decoder-Only架构与完整的Encoder-Decoder架构有什么区别？为什么现在主流的大语言模型都采用Decoder-Only架构？

     你的答案：有编码器没有解码器，目的是预测下一个token，解码器多余了

     点评：这题这里答反了。Decoder-Only 的意思是“只有解码器，没有编码器”，不是“有编码器没有解码器”。不过你后半句抓到了核心：它主要用于预测下一个 token。

     参考答案：完整的 Encoder-Decoder 架构中，编码器负责理解输入序列，解码器负责在参考编码器输出的基础上逐步生成目标序列，因此很适合翻译、摘要等标准序列到序列任务。
     Decoder-Only 架构则只保留解码器，通过掩码自注意力在“只能看历史、不能看未来”的条件下进行自回归生成，也就是根据前文预测下一个 token。
     现在主流的大语言模型广泛采用 Decoder-Only 架构，主要是因为它与“海量文本上做下一个 token 预测”的训练目标天然一致，训练目标统一、架构简洁、扩展性强，并且非常适合聊天、续写、代码生成等生成类任务。

3. 文本子词分词算法是大语言模型的一项关键技术，负责将文本转换为模型可处理的 token 序列。那为什么不能直接以"字符"或"单词"作为模型的输入单元？BPE（Byte Pair Encoding）算法解决了什么问题？

   你的答案：按单词分这样词汇量是巨大的，而且模型将无法处理任何未在词汇表出现的词，按字符的话单个字符大多不具有独立语义，模型需要学习如何将字符组合成有意义的词，BPE控制了词的大小，又能让模型通过组合字词来理解和生成新词

   点评：这题答得不错，已经覆盖了词级分词和字符级分词的主要问题，也说清楚了 BPE 的折中思想。可以再明确一点：BPE 通过高频子词合并来兼顾词表规模和表达能力。

   参考答案：如果直接按单词分词，词表会非常大，而且会频繁遇到未登录词（OOV）问题，模型无法自然处理训练时没见过的新词。如果直接按字符分词，虽然词表很小、不容易出现 OOV，但单个字符通常语义过弱，模型需要花更多成本去学习字符如何组合成有意义的词语，学习效率较低。
   BPE 的核心作用是通过频繁合并高频相邻子词，构建一套“子词级”词表，从而在控制词表规模的同时保留一定的语义单元表达能力。这样模型既能把常见词作为整体处理，也能通过组合子词来理解和生成未见过的新词。
...
4. 本章3.2.3节介绍了如何本地部署开源大语言模型。请完成以下实践和分析：
   ...
   - 按照本章的指导，在本地部署一个轻量级的开源模型（推荐[Qwen3-0.6B](https://modelscope.cn/models/Qwen/Qwen3-0.6B)），并尝试调整采样参数并观察其对输出的影响
     ...
     参考答案（示例作答）：我选择部署轻量级开源模型 `Qwen3-0.6B`，使用本地 Python 环境与 `transformers` 完成推理。实验中主要调整了 `temperature`、`top_p` 和 `max_new_tokens` 三类参数。
     观察结果如下：
     当 `temperature` 较低（如 0.2）时，模型输出更稳定、更保守，适合事实问答或结构化任务；当 `temperature` 较高（如 0.9）时，输出更发散、更有创造性，但更容易出现不稳定和幻觉。
     当 `top_p` 较低时，模型会从更小的高概率候选集合中采样，输出更稳；当 `top_p` 提高时，模型探索性增强，语言风格更丰富。
     `max_new_tokens` 决定输出上限，值过小会导致回答被截断，值过大则可能增加冗余内容和推理成本。
     因此，不同任务需要不同采样配置：事实型任务更适合低温度、较低 `top_p`；创意生成任务更适合更高温度和更宽松的采样范围。

   - 选择一个具体任务（如文本分类、信息抽取、代码生成等），设计并对比以下不同的提示策略（如Zero-shot、Few-shot、Chain-of-Thought）对输出结果的效果差异

     参考答案（示例作答）：我选择“商品评论情感分类”任务，目标是将评论分类为“正向、负向、中性、混合”四类。
     对于测试文本：
     “这款耳机音质确实不错，降噪也够用，但戴久了夹耳朵，而且续航没有宣传得那么久。”
     使用 Zero-shot 提示时，直接要求模型判断情感类别。模型通常能够判断为“混合”，但输出格式不稳定，且对边界标签的理解可能不一致。
     使用 Few-shot 提示时，我在 prompt 中先提供两条输入输出示例，再让模型处理测试评论。此时模型对“混合情感”的判断更稳定，输出格式也更统一。
     使用 Chain-of-Thought 提示时，我要求模型先分步列出正面信息和负面信息，再做综合判断。模型通常会明确指出“音质不错、降噪够用”为正面信息，“夹耳朵、续航不足”为负面信息，最后得出“混合”这一更可解释的结果。
     综合来看，Zero-shot 成本最低但稳定性一般；Few-shot 在格式和边界理解上表现更好；CoT 在复杂样本中最有助于提升可解释性和稳健性，但 token 成本也更高。

   - 从性能、成本、可控性、隐私等维度比较闭源模型和开源模型

     参考答案：从性能上看，闭源模型通常在综合能力、推理稳定性和工具生态上更强，尤其在复杂推理和多模态方面往往领先；开源模型近年来进步很快，在中小规模任务和定制场景中已经具备较强可用性。
     从成本上看，闭源模型通常按调用量计费，适合快速试验，但高并发和长期使用成本可能较高；开源模型需要承担本地部署、推理硬件和运维成本，但在高频调用场景下可能更划算。
     从可控性上看，开源模型更有优势，开发者可以自行微调、部署、蒸馏和修改推理流程；闭源模型则更依赖服务提供商的接口和策略。
     从隐私与合规角度看，开源模型支持私有化部署，更适合处理敏感数据；闭源模型需要额外关注数据出境、日志留存和企业合规要求。

   - 如果你要构建一个企业级的客服智能体，你会选择哪种类型的模型？需要考虑哪些因素？

     参考答案：如果是企业级客服智能体，我更倾向于“闭源强模型 + 检索增强 + 规则约束”或“可私有化部署的强开源模型 + 企业知识库”两种路线中的一种，具体选择取决于业务场景和合规要求。
     如果企业对数据安全和私有化要求极高，我会优先考虑可本地部署的开源模型，并结合 RAG、知识库、意图识别和人工兜底机制。
     如果企业更看重上线速度、基础能力和维护成本，且数据合规允许，可以优先使用闭源模型 API。
     选择时需要重点考虑：模型效果、延迟、成本、知识可更新性、隐私合规、调用稳定性、工具调用能力、多轮对话能力、幻觉控制能力以及与现有客服流程的集成成本。

5. 模型幻觉（Hallucination）<sup>[11]</sup>是大语言模型当前存在的关键局限性之一。本章介绍了缓解幻觉的方法（如检索增强生成、多步推理、外部工具调用）

   - 请选择其中一种，说明其工作原理和适用场景

     参考答案：我选择检索增强生成（RAG）作为示例。其核心思想是：在模型生成答案之前，先根据用户问题从外部知识库中检索出相关文档片段，再把这些片段与原始问题一起输入模型，让模型基于检索到的证据生成回答。
     它的工作流程一般包括：文档切分与向量化、向量检索召回相关内容、可选的重排序、将检索结果拼接进 prompt、最后由模型生成答案。
     RAG 的优势在于能够把回答建立在外部知识证据之上，从而减少模型凭参数“编答案”的概率，也便于知识更新。它特别适合知识密集型问答、企业知识库客服、文档问答、政策解读、产品说明等场景。

   - 调研前沿的研究和论文，是否还有其他的缓解模型幻觉的方法，他们又有哪些改进和优势？

     参考答案：除了基础的 RAG、多步推理和工具调用，近年的研究还提出了多种改进方法。
     第一类是检索增强方法的升级，例如 Corrective RAG（CRAG）和 Self-RAG。这类方法不只是“检索后直接生成”，而是会对检索质量进行评估、纠错或让模型主动决定何时检索、如何反思生成结果，因此比传统 RAG 更稳健。
     第二类是生成后验证方法，例如 Chain-of-Verification（CoVe）。这类方法先生成初步答案，再引导模型对关键事实做逐条核查，从而减少未经验证的断言。
     第三类是采样与推理一致性方法，例如 Self-Consistency。它通过对同一问题采样多条推理路径，再选择一致性更高的结果，用于降低单条错误推理导致的幻觉。
     第四类是训练阶段的对齐和事实性增强，例如基于人类反馈或可验证奖励的后训练方法，让模型更倾向于输出可证实的答案、拒绝无根据猜测。
     这些方法的共同趋势是：不再单纯依赖模型参数记忆，而是把检索、验证、反思和外部工具结合进生成流程中，从而提高事实一致性与可靠性。
...
6. 假设你要设计一个论文辅助阅读智能体，它能够帮助研究人员快速阅读并理解学术论文，包括：总结论文研究的核心内容、回答关于论文的问题、提取关键信息、比较多篇不同论文的观点等。请回答：
   ...
   - 你会选择哪个模型作为智能体设计时的基座模型？选择时需要考虑哪些因素？
     ...
     参考答案：如果设计论文辅助阅读智能体，我会优先选择上下文窗口较长、指令遵循较好、学术文本理解能力较强的模型作为基座。如果企业和项目允许使用闭源模型，我会优先考虑强推理能力的闭源模型；如果需要私有化部署或成本受限，则会选择能力较强的开源模型并结合 RAG。
     选择时需要考虑的因素包括：长上下文能力、学术英文与术语理解能力、总结与问答质量、推理稳定性、引用与结构化输出能力、调用成本、推理延迟、隐私合规以及与外部知识库和工具系统的集成能力。

   - 如何设计提示词来引导模型更好地理解学术论文？学术论文通常很长，可能超过模型的上下文窗口限制，你会如何解决这个问题？

     参考答案：在提示词设计上，我会明确要求模型区分论文的研究问题、方法、实验设置、核心贡献、局限性与结论，并要求输出结构化结果，例如按“问题、方法、实验、结果、局限”几个字段组织答案。
     对于超长论文，一种可行方案是分块处理与分层总结。首先按摘要、引言、方法、实验、结论等章节切分论文；然后让模型分别对每一部分做局部总结；最后再做全局汇总。进一步可以结合 RAG，让用户提问时先检索最相关的段落，再让模型基于这些段落回答，从而降低上下文溢出和无关内容干扰。

   - 学术研究是严谨的，这意味着我们需要确保智能体生成的信息是准确客观忠于原文的。你认为系统中加入哪些设计能够更好的实现这一需求？

     参考答案：为了保证论文辅助阅读智能体的输出准确、客观、忠于原文，我会加入以下设计。第一，基于原文证据回答，输出中尽量附带引用段落或页码定位。第二，使用 RAG 或原文检索，避免仅凭模型参数记忆作答。第三，对关键事实设置验证步骤，例如让模型在回答后回查原文证据。第四，对于无法从原文明确支持的内容，要求模型明确标注“不确定”或“原文未提及”。第五，对跨论文比较任务采用结构化对比模板，减少随意发挥。第六，对高风险输出增加人工复核或双模型交叉检查机制。
